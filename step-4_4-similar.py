...
for word, vector in data.items():
    if word != target_word:
        ...
        if len(top_10_words) < heap_size:
            heapq.heappush(top_10_words, (similarity, word))
        else:
            heapq.heappushpop(top_10_words, (similarity, word))

most_similar_words = [(word, similarity) for similarity, word in heapq.nlargest(heap_size, top_10_words)]
