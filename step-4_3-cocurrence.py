...
for word, vector in data.items():
    if word != target_word:
        ...
        if len(top_10_words) < heap_size:
            heapq.heappush(top_10_words, (correlation, word))
        else:
            heapq.heappushpop(top_10_words, (correlation, word))

most_correlated_words = [(word, correlation) for correlation, word in heapq.nlargest(heap_size, top_10_words)]
